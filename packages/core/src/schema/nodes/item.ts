...
/**
 * Returns the effective world-space dimensions of a node after applying its scale/config.
 * Supports ItemNode, ColumnNode, and RackNode.
 */
export function getScaledDimensions(node: any): [number, number, number] {
  ...
  if (node.type === 'column') {
    const col = node as any
    return [col.width ?? 0.4, col.height ?? 3.0, col.depth ?? 0.4]
  }

  if (node.type === 'rack') {
    const rack = node as any
    const units = rack.units?.length ?? 1
    const w = (rack.unitWidth ?? 2.7) * units
    const h = rack.unitHeight ?? 6.0
    const d = rack.unitDepth ?? 1.1
    return [w, h, d]
  }
