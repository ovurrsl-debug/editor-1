...
const isNodeOnLevel = (node: AnyNode, levelId: string): boolean => {
  ...
  // Rack/Column attached to level
  if ((node.type === 'rack' || node.type === 'column') && node.parentId === levelId) {
    return true
  }
...
const isNodeInZone = (node: AnyNode, levelId: string, zoneId: string): boolean => {
  ...
  // Special types that are always valid once level is confirmed
  if (node.type === 'roof' || node.type === 'roof-segment' || node.type === 'rack' || node.type === 'column') {
    return true
  }
...
const getStrategy = (): SelectionStrategy | null => {
  ...
  // Level selected, no zone -> can select zones or structural items (racks, columns)
  if (!zoneId) {
    return {
      types: ['zone', 'rack', 'column'],
      ...
      isValid: (node) =>
        (node.type === 'zone' || node.type === 'rack' || node.type === 'column') &&
  ...
  return {
    types: ['wall', 'item', 'slab', 'ceiling', 'roof', 'roof-segment', 'window', 'door', 'rack', 'column'],
    handleClick: (node, nativeEvent) => {
